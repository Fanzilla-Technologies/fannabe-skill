...
```bash
fannabe models options --filter topic  --media-type image   # Themes (e.g. mirror-selfie, dubai)
fannabe models options --filter outfit --media-type image   # Outfits (e.g. casual, gym)
...
Using a `locationPrompt` or `outfitPrompt` automatically switches the request to simple mode so the preset actually applies. The ids come from the `models options` listing.
